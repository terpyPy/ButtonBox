def gameLogic(theBoard, event, onColor, offColor):
    ...
    neighborsLit = 0
    allLit = False
    neighborsSize = len(neighbors[keyPressed])
    for j in range(neighborsSize):
            if newBoard[neighbors[keyPressed][j]] == onColor:
                neighborsLit += 1
    if neighborsLit == neighborsSize:
        allLit += True
    # get the range length of the list of neighbor cells 
    for i in range(neighborsSize):
